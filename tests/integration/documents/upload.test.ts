...
import { saveContentAddressedPdf, validatePdfUpload } from "../../../apps/api/src/document-ingest/upload.js";
...
describe("PDF upload boundary", () => {
  it("validates multipart metadata and saves idempotently by content hash", async () => {
    ...
    const input = { filename: "paper.pdf", contentType: "application/pdf", bytes };
  ...
  it("rejects non-PDF multipart input", () => {
    expect(() => validatePdfUpload({ filename: "paper.txt", contentType: "text/plain", bytes: Buffer.from("hello") })).toThrowError(expect.objectContaining({ code: "UNSUPPORTED_INPUT" }));
  });
});
