...
const checkWave0Boundaries = () => {
  ...
  for (const relative of forbidden) {
    if (fs.existsSync(path.join(root, relative))) fail(`Wave 0 boundary violated: ${relative}`);
  }
};
...
const runHealth = async () => {
  ...
  try {
    ...
    if (!response) throw new Error(`health endpoint did not start; output=${output}`);
    const body = await response.json();
    if (response.status !== 200 || body.status !== 'ok' || body.wave !== 0) {
...
if (['format', 'lint', 'typecheck', 'build', 'security', 'all'].includes(mode)) {
  // Wave 0 intentionally has no third-party formatter, linter, compiler, or scanner.
  // This script verifies the structural gates that are available without dependencies.
