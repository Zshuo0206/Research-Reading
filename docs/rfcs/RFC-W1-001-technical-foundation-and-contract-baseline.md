...
- 持久化模型配置只含 provider、base_url、model、超时和输入/输出上限；真实 BYOK request 顶层只允许一个 invocation-only `runtime_secret_ref`，Mock request 不带 secret ref，Schema 不含明文密钥字段。
- ModelGateway 使用完整的 `REQUEST/RESPONSE + operation` discriminated union。QuestionPlanDraft/AnswerDraft 不含持久化 ID、revision、时间、hash 或审核状态；模型只能返回问题/回答草稿和服务端生成的 `context_span_id` 候选引用，不得生成最终 `char_start`/`char_end`。最终正式对象和 EvidenceSpan 由服务端物化、计算和验证。
