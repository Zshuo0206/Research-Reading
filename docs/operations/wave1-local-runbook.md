...
每次 start 都在已忽略的 `tmp/v1-local/runs/<run-id>/` 下创建独立日志和空的 Worker stop control file 路径，并在 `tmp/v1-local/processes.json` 中记录本次 run id、PID、启动时间、路径和 `lifecycle_status`。状态文件不保存 secret。

启动顺序固定为 API → API health → Web → Web health → Worker → Worker ready。API 或 Web 尚未健康时不会创建 Worker。API、Web、Worker 创建后，state 分别以 `STARTING_API`、`STARTING_WEB`、`STARTING_WORKER` 写入；当前 run 的 stdout 解析到合法 Worker ready 后立即以 `worker_ready_observed=true` 原子更新 state，但整体健康确认完成前仍保持 `STARTING_WORKER`，随后才写 `READY`。state 使用同目录 `processes.json.tmp` 完整写入后再原子替换活动文件：
...
start 必须逐行解析 Worker stdout 的真实 JSON，只有看到当前进程输出的 `ready`、`accepts_jobs: true` 和完整 readiness 标记才会成功；仅有存活 PID 不算 ready。ready 日志是 `worker_ready_observed` 的权威事实，即使 Worker 随后立即退出或 PowerShell 尚未完成正常返回，也按 ready 后故障处理。check 使用相同规则，并将 retry 事件报告为 degraded、错误或终止事件报告为 unhealthy。

start 会区分 Worker ready 前后。Worker 尚未创建时，只清理已记录且 ownership 匹配的 API/Web。Worker 已创建但从未产生合法 ready 时，它尚未进入 Job loop：如果仍活着，start 先创建 control file 并等待其退出；如果已自行以 `worker_start_failed` 退出，则无需等待不可能产生的 `CONTROL_FILE` stopped 事件，随后安全清理 API/Web 和活动 state，并保留 run 日志。Worker 已合法 ready 后，回滚仍必须创建 control file、等待当前 Job 完成和 `CONTROL_FILE` stopped，再按 API → Web 清理；超时会返回非零并保留 Worker、API、Web、state 和日志。

若 ready Worker 已死且没有 `CONTROL_FILE` stopped，state 进入 `CRASHED_WORKER_REVIEW_REQUIRED`，因为 SQLite 中可能存在 orphan RUNNING Job。普通 stop 会拒绝清理。核对日志和 orphan 风险后，操作员可显式运行：

```powershell
.\scripts\stop-v1-local.ps1 -AcknowledgeCrashedWorker
```

该命令只在 Worker PID 已死、ready 已记录、无 stopped 事件且 API/Web ownership 记录存在时执行；仍存活的 API/Web 必须通过 PID+启动时间校验，已自行停止的 owned API 或 Web 会输出 `already stopped` 并跳过。命令把原 state 归档到本次 run 的 `crashed-worker-state.json`，不修改或恢复 Job，不删除 SQLite/content/logs，也不代表 graceful Worker stop。PID 复用仍会拒绝操作。
...
真实 Windows managed process smoke 是独立 Release Gate，不属于标准 `npm test` 或通用 `ci`：

```powershell
npm run test:v1-managed-windows
```

该命令会先构建，再从 `tests/release-gate/v1-local-managed-smoke.test.ts` 实际执行 10 个场景：正常 start/check/stop、ready 前 Worker 自行失败、API-only partial startup、API/Web partial startup、真实 JobRuntime claimed-job 回滚、回滚超时、ready-before-observation 崩溃竞态、API 已停止和 Web 已停止的 partial crashed-worker cleanup，以及 ready 后无 stopped 事件的 Worker crash/显式 cleanup。测试使用独立临时 SQLite/content/entrypoint，不覆盖正式 Worker dist。非 Windows 明确 skip；固定端口被外部进程占用时明确失败且不终止占用者。清理失败时保留本次 run 日志并报告目录，不无条件删除诊断证据。
...
- check 显示 `unhealthy interrupted startup`：`STARTING_*` state 中至少一个已记录 owned PID 已停止或发生 PID 复用；先保留 run 目录和日志，核对 ownership，不要终止非 state 所有的进程。
- stop 超时：当前 Job 可能仍在运行；保留现有进程和状态，查看日志并等待后重试，不强制终止 Worker。
- start 显示 `START_FAILED_STOP_PENDING`：启动回滚正在等待 Worker 完成当前 Job；不要强杀 Worker，稍后重跑 stop。
- check 显示 `crashed worker review required`：ready Worker 已异常退出且缺少 `CONTROL_FILE` stopped；先检查 run 日志和 SQLite orphan RUNNING 风险。普通 stop 不会清理；只有明确接受该风险后才使用 `-AcknowledgeCrashedWorker`，该命令不会恢复 Job。
