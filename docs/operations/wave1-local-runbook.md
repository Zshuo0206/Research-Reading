...
启动顺序固定为 API → API health → Web → Web health → Worker → Worker ready。API 或 Web 尚未健康时不会创建 Worker。Worker 创建后、等待 ready 前，state 已以 `STARTING` 写入；三项全部健康后才更新为 `READY`：
...
start 在 Worker 创建后的任何失败也不得直接强制终止 Worker。它会把状态更新为 `START_FAILED_STOP_PENDING`、创建本次 control file、等待当前 Job 完成和 `CONTROL_FILE` stopped 事件，然后才按 API → Web 清理。等待超时会返回非零并保留 Worker、API、Web、state 和日志；稍后应重新运行 stop。Worker 尚未创建的启动失败才允许按 PID+启动时间直接清理本脚本创建的 API/Web。
...
该命令会先构建，再从 `tests/release-gate/v1-local-managed-smoke.test.ts` 实际执行正常 start/check/stop、Worker 创建后启动失败的 control-file 回滚，以及回滚超时保留 state/API/Web。非 Windows 明确 skip；固定端口被外部进程占用时明确失败且不终止占用者。清理失败时保留本次 run 日志并报告目录，不无条件删除诊断证据。
