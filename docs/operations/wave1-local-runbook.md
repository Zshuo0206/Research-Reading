...
启动顺序固定为 API → API health → Web → Web health → Worker → Worker ready。API 或 Web 尚未健康时不会创建 Worker。API、Web、Worker 创建后，state 分别以 `STARTING_API`、`STARTING_WEB`、`STARTING_WORKER` 写入；只有解析到合法 Worker ready 后，才以 `worker_ready_observed=true` 更新为 `READY`。state 使用同目录 `processes.json.tmp` 完整写入后再原子替换活动文件：
...
该命令只在 Worker PID 已死、ready 已记录、无 stopped 事件且 API/Web ownership 仍匹配时停止 owned API/Web，并把原 state 归档到本次 run 的 `crashed-worker-state.json`。它不修改或恢复 Job，不删除 SQLite/content/logs，也不代表 graceful Worker stop。
...
该命令会先构建，再从 `tests/release-gate/v1-local-managed-smoke.test.ts` 实际执行 7 个场景：正常 start/check/stop、ready 前 Worker 自行失败、API-only partial startup、API/Web partial startup、真实 JobRuntime claimed-job 回滚、回滚超时，以及 ready 后无 stopped 事件的 Worker crash/显式 cleanup。测试使用独立临时 SQLite/content/entrypoint，不覆盖正式 Worker dist。非 Windows 明确 skip；固定端口被外部进程占用时明确失败且不终止占用者。清理失败时保留本次 run 日志并报告目录，不无条件删除诊断证据。
