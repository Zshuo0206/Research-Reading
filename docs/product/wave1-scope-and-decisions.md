# Wave 1 Product Scope and Decision Topics

状态：`PRODUCT_SCOPE_APPROVED_V1_BASELINE_RECORDED_TECHNICAL_REFINEMENT_PENDING`
用途：人类项目负责人已正式批准的 Wave 1 产品范围；允许技术方案、契约和任务规划，不授权开始真实业务编码。

一页式摘要见：[wave1-product-scope-summary.md](D:/Research%20Reading/docs/product/wave1-product-scope-summary.md)

V1.0 产品定义归档见：[versions/科研文献引导式学习平台_V1.0产品定义.md](./versions/科研文献引导式学习平台_V1.0产品定义.md)；开发执行基线见：[v1.0-development-requirements.md](./v1.0-development-requirements.md)。

## 一、已批准的产品范围

### 1. 产品定位

- 面向个人研究者。
- 形态为单用户 Web 工作台。
- 暂不支持课题组协作、插件、本地部署和私有部署。

### 2. 阅读模式

- Wave 1 的“方法学习”包含两个独立入口：快速问答模式和 V1.0 引导式学习模式。
- 快速问答保留问题确认与带证据回答的快速路径；引导式学习是 V1.0 的核心产品路径。
- 引导式学习 V1.0 只开放“理解内容”阶段，分析评价和迁移应用只展示锁定态。
- 写作支持、课题决策和实验设计只保留为未来扩展边界，不进入本轮真实实现。

### 3. 论文输入

- 允许导入一篇真实、可直接提取文本的 PDF。
- 扫描 PDF、OCR、复杂表格和公式理解明确排除。
- 建立少量固定评测论文，用于稳定验收和回归测试。

### 4. 模型使用

- 允许通过可替换的 `ModelGateway` 使用一个外部模型。
- 必须同时保留 Mock 实现，用于自动测试、失败测试和不依赖供应商的验收。
- 用户必须被告知论文内容是否发送至外部模型。
- 必须定义数据发送边界、成本上限、日志脱敏和外部服务失败处理。
- 供应商选择、适配层和具体实现由技术角色提出方案并完成技术验收；只有明显影响产品范围、隐私、成本或长期形态时才提交人类项目负责人决定。

### 5. V1.0 引导式学习产品闭环

V1.0 在第一轮基础能力之上增加以下产品要求：

- 用户输入学习目标，系统生成 2–3 个候选学习方向，用户选择一个方向；V1.0 不支持编辑候选方向。
- 系统固定展示“理解内容、分析评价、迁移应用”三阶段路线，只有第一阶段可进入。
- 第一阶段一次生成 3–7 道开放式问题，按顺序逐题展示，不要求用户提前确认完整题目清单。
- 每题支持自由文本作答或“不会/跳过”，提交后显示简短点评、论文内参考答案和可定位证据；用户可修正当前回答并主动进入下一题。
- 完成第一阶段后生成简短阶段总结；不生成评分、掌握等级、学习报告或回答版本历史。

实现映射和边界见 `docs/product/v1.0-development-requirements.md`；公共契约与状态机变更必须先经 `RFC-W1-002` 技术审查。

## 二、Wave 1 最小产品闭环

Wave 1 至少覆盖以下流程：

> 创建研究项目 → 导入一篇论文 → 选择“方法学习” → 生成结构化问题计划 → 用户确认、修改或拒绝问题 → 对至少一个确认问题生成回答 → 展示原文证据片段及页码或稳定文本位置 → 用户确认、修改或拒绝回答。

边界：

- 不要求一次完成全部问题。
- 不要求高级 Evidence 检索、多篇综合、正式研究资产库或 Markdown/Excel 导出。
- 未经用户确认的回答只能是草稿，不得视为正式研究资产。

## 三、科研可信度要求

每个回答必须明确区分：

- `PAPER_FACT`：论文中可以核验的事实；
- `AUTHOR_CLAIM`：作者的主张或解释；
- `AGENT_INFERENCE`：Agent 基于证据作出的推断。

证据回答必须能够回溯到：

- 论文文件版本；
- 页码或稳定文本位置；
- 支撑回答的原文片段。

缺少可回溯证据的事实性回答不能通过产品验收。

## 四、产品验收标准

### 必须满足

1. 单用户可以完成上述最小产品闭环。
2. 只支持“方法学习”，其他阅读模式不会误进入真实流程。
3. 真实可提取文本 PDF 可以被识别、导入并关联文件版本。
4. Mock ModelGateway 和外部模型适配路径具有相同的结构化输出契约。
5. 至少一个确认问题能够生成回答草稿。
6. 回答展示证据片段以及页码或稳定文本位置。
7. 用户可以修改、拒绝问题计划和回答。
8. 回答区分论文事实、作者主张和 Agent 推断。
9. 未确认回答不会进入正式研究资产。
10. 固定评测论文能够稳定复现关键验收场景。

### 主要产品指标

- 问题可用率；
- 带证据回答的用户接受率。

### 辅助指标

- 问题修改率；
- 回答拒绝原因；
- 任务完成时间；
- 用户是否认为该流程比普通摘要更有帮助。

## 五、明确排除范围

- 写作支持、课题决策和实验设计的真实实现。
- 扫描 PDF、OCR、复杂表格和公式理解。
- 高级 Evidence 检索、多篇文献综合。
- 正式研究资产库、Markdown/Excel 导出。
- 多用户协作、课题组功能、权限体系。
- Zotero、Word、LaTeX、浏览器插件。
- 本地部署、离线产品、本地模型和私有部署。
- 自动判断创新性、研究空白或论文质量。
- V1.0 的 OCR、动态难度、追问、评分、掌握等级、完整学习历史、复杂总结、移动端和 UI 精修。
- 未经再次审核的额外产品模块。

## 六、技术方案状态

技术角色已完成第一版论证，见 [`docs/architecture/wave1-technical-plan.md`](D:/Research%20Reading/docs/architecture/wave1-technical-plan.md)。当前建议采用 Node 22/TypeScript、Fastify、React/Vite、SQLite、内容寻址文件存储、按页 PDF 文本提取、独立 Worker、JSON Schema/Ajv 和可替换 ModelGateway。契约提案见 [`RFC-W1-001`](D:/Research%20Reading/docs/rfcs/RFC-W1-001-technical-foundation-and-contract-baseline.md)，当前为 `PROPOSED`，尚未冻结。

技术方案已覆盖：

- 前端/后端运行时与技术栈；
- 外部模型适配层和 Mock 策略；
- PDF 文本提取、文件版本、稳定文本位置和不支持格式识别；
- 数据发送边界、日志脱敏、成本计量、预算阈值和外部服务失败处理；
- 数据库、内容寻址文件存储和 SQLite job worker；
- API、错误模型、任务状态、答案类型和证据 Schema；
- 固定评测论文、人工基准、问题/回答评测方法；
- formatter、lint、typecheck、依赖和安全工具的 CI 接入；
- 独立 QA Agent、责任 Agent 局部检查和主控复跑方式。

实现解释：产品范围允许使用可替换的外部 `ModelGateway`，Wave 1 同时冻结版本化 Gateway 契约、确定性 Mock 和统一 OpenAI-compatible BYOK 适配器。OpenAI、Gemini、Groq、OpenRouter 以预设提供，自定义端点必须为 HTTPS；真实调用只进入人工验收，不进入普通 CI。模型设置页面属于后续 Web 闭环任务。

## 七、本轮人工批准结论

1. 正式批准“个人单用户 Web 工作台”作为 Wave 1 产品形态。
2. 正式批准一篇真实、可直接提取文本的 PDF 和可替换外部 `ModelGateway` + Mock 进入 Wave 1。
3. 正式认可“问题可用率”和“带证据回答的用户接受率”为主要产品指标。
4. 正式接受外部模型必须经过用户告知/同意、最小发送、日志脱敏、预算阈值和失败处理。
5. 正式接受 Wave 1 只实现方法学习，其他模式延期。
6. 正式接受本轮不实现正式资产库、导出、多篇综合、高级 Evidence 检索、OCR、协作和插件。
7. V1.0 产品定义作为后续开发需求基线归档；其公共契约、状态机和数据库影响仍须按 RFC-W1-002 完成技术审查，不因产品文档归档自动授权编码。

后续只有在技术实现改变已批准闭环、扩大/缩小范围、显著改变论文隐私或持续成本、产生供应商锁定，或改变个人 Web 工作台长期定位时，才重新提交人类项目负责人决定。

## 八、技术方案与编码启动条件

在以下条件完成前，不得分配或开始 Wave 1 真实业务编码：

- 人类项目负责人已确认本文件和一页式摘要中的产品范围（已完成）。
- 主控 Agent 已建立技术方案、ownership、契约版本、任务、验收标准和回滚方案（已提出，待冻结）。
- 技术角色已完成 ModelGateway、PDF 输入、证据回溯、成本/隐私和评测方案论证（已提出，待技术审查）。
- QA 执行方式已明确为独立 QA Agent + 主控复跑，并写入技术方案和任务验收标准（已提出，待确认 owner）。
- 技术方案、RFC 和契约基线冻结后，人类项目负责人还需单独明确允许开始 Wave 1 真实业务编码。
