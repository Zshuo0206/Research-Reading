...
- Evidence 使用本次 canonical context span 的唯一 quote 精确匹配，生成服务端 page number、code-point offset、page hash、profile 和 verification；无效/未知/重复 quote 降级为 `INSUFFICIENT_EVIDENCE`，不伪造 span。
...
| `npm test` | 通过；24/24 |
| `npm run e2e:smoke` | 通过；3/3，含 BYOK-shaped UI、Mock 和刷新恢复 |

专项 fake provider 覆盖四类模型 operation、provider 持久化、唯一 quote、Unicode/code-point offset、无效 quote 无 Evidence、INSUFFICIENT_EVIDENCE claim 无引用、PDF endpoint、非法页码和客户端 secret/ref 不接受。
