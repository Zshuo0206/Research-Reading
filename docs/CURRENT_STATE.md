...
`guided-learning.v1` runtime 已进入 main；T-W1-013 已进入 main，T-W1-014 Worker 生成已集成 main。后续 Web、E2E 和验收
任务仍需按依赖单独授权。

## main 当前基线

- `main`、`origin/main` 和本任务开发基线为 `5b6c29c14f5ccb6d524004565a36fa761cd800a2`，已包含 T-W1-014 Worker 集成。
...
- T-W1-015 分支新增 Guided Learning Web 交互；尚未合并 main。
...
- T-W1-015：`REVIEW`，当前分支完成 Guided Learning Web 入口、目标/PDF/Session 创建、轮询恢复、方向选择、逐题反馈、Evidence、重试和阶段总结；尚未合并 main。
...
- T-W1-015 分支验证：Web API client/轮询 5/5，Guided Learning runtime + Worker 12/12，Playwright Web smoke 3/3（含快速问答回归和 Guided Learning Mock-shaped 流程），真实 `synthetic-text.pdf` 后端 Mock 闭环保持通过；BYOK 人工验收尚未执行。
...
- T-W1-015 尚未合并 main；Wave 1 真实 PDF 完整 E2E、真实 BYOK 人工验收和完整产品验收尚未完成。
...
1. 将 T-W1-015 Guided Learning Web Workbench 独立审查并集成 main。
2. 使用真实可提取文本 PDF 完成 Wave 1 完整 E2E，再执行 BYOK 人工验收和完整产品验收。
