...
1. 增加 per-claim 安全 Evidence resolution reason diagnostics，定位全部 claims 降级为 INSUFFICIENT_EVIDENCE 的具体分类并修复 Evidence grounding；不放宽 source verification，不绕过 confirm gate。
2. 在出现 VERIFIED Evidence 后完成 Evidence 人工抽查，并由人类项目负责人决定是否进入 `READY_FOR_V1_0_TAG`；不自动创建 tag。
