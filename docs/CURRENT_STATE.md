...
状态：`WAVE_1_GUIDED_LEARNING_WORKER_IMPLEMENTED_WEB_PENDING`
...
`guided-learning.v1` runtime 已进入 main；T-W1-013 已进入 main，T-W1-014 在当前隔离分支实现、尚未集成。后续 Web、E2E 和验收
任务仍需按依赖单独授权。

## main 当前基线

- `main`、`origin/main` 和当前开发基线为 `7f7684d8ed6107b10bb3d8091d45984a240feb90`。
- PR #2、PR #3、PR #4 已合并；PR #4 合并了 `guided-learning.v1` 契约实现。
- T-W1-011 已集成；T-W1-013 API/SQLite runtime 已进入 main。T-W1-014 在当前隔离分支实现，其代码不计入 main 当前能力。

## 当前 Wave

Wave 1 已进入基础运行时和公共契约集成阶段。快速问答模式继续保留，
`guided-learning.v1` 和 T-W1-013 API/SQLite runtime 已进入 main；T-W1-014 Worker 在当前隔离分支推进。
...
main 已包含 API/SQLite runtime；当前分支额外包含 T-W1-014 Worker，但尚未集成 main：

- 引导式学习 API Session/Command 持久化和 HTTP runtime；
- 引导式学习专用数据库表、字段和 migration；
- 候选方向、路线、逐题问题、点评、参考答案和阶段总结的 Worker 生成（T-W1-014 分支能力）；
- 引导式学习 Web 交互；
- 真实 PDF + 引导式学习端到端闭环验收。
...
- T-W1-013 已进入当前 main 基线；T-W1-014 仍在隔离分支，不把其分支代码计入 main 当前能力。
...
- T-W1-014：`REVIEW`，当前分支完成四类 Worker Job、原子入队、真实 PDF Mock 闭环和失败重试；尚未进入 main。
...
- T-W1-014 Worker 仍待独立审查和集成；Web 交互、真实 BYOK 人工验收和完整产品验收尚未完成。
...
1. 审查并集成 T-W1-014 的 Worker 生成路径。
2. 实现目标输入、方向选择、路线、逐题学习和阶段总结 Web 交互。
3. 使用真实可提取文本 PDF 完成集成验收，再执行 BYOK 人工验收和完整产品验收。

不自动扩大 T-W1-005/T-W1-006 的范围；T-W1-014 保持独立隔离，不因本次开发而宣称整个 Wave 1 完成。
